...
read:
   lcall getbyt           ; get address high byte
   mov dph, a             ; get the first half
   lcall prthex           ; print the first half
   lcall getbyt           ; get address low byte
   mov dpl, a             ; get the second half of the address
   lcall prthex           ; print the second half
   lcall crlf             ; get a newline 
   movx a, @dptr          ; move entirety of dptr into a
   lcall prthex           ; print out the contents of the accumulator to terminal
...
write:
   ...
   lcall prthex           ; print the second half
   lcall getchr
   lcall sndchr           ; get the equals sign
   lcall getbyt
   lcall prthex
   movx @dptr, a          ; move entirety of dptr into a
   lcall crlf             ; get a newline 
   ljmp endloop           ; do jump by doing a ret
;===============================================================
; command table  't'
...
table:
   ...
   mov dph, a             ; get the first half
   lcall prthex           ; print the first half
   mov a, 00h             ; second half is always zero
   mov dpl, a             ; move 00 into the low byte
   lcall crlf             ; get a newline 
   mov R0, #0FFh          ; Set the 256 byte counter in R0
   rdata:
        mov a, dph             ; Move high hyte into acc
        lcall prthex           ; print high byte to serial
        mov a, dpl             ; Move low byte into acc
        lcall prthex           ; Print low byte 
        mov a, #20h            ; Move hex code for a space in ascii
        lcall sndchr           ; Add Space between the address and the written byte
        lcall getbyt           ; get the given byte
        lcall prthex           ; Print the given byte to serial
        mov R1, a      
        movx @dptr, a          ; Write that byte to the location given byt dptr
        lcall crlf             ; Get a newline
        inc dpl                ; increment the low byte and start over
        DJNZ R0, rdata         ; Decrement R0, that counts 256 entered bytes
   ljmp endloop                ; End loop at 256 entered bytes
