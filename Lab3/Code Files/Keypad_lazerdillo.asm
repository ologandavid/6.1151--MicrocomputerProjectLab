...
main:
    ...
    mov IE, #82h
    loop:
        lcall getkey    ; Get key and update the Registers based on the selection
        sjmp loop       ; Loop Back
...
getkey:
    ...
    mov a , P1              ; move P1 into acc
    clr C               
    SUBB A, #0F0h           ; Make first nibble 0s
    mov P1, a               ; 
    clr TR0                 ; Stop Clock Momentarily
    B1: ; TIE Fighter
        CJNE a, #0Fh, B2
    ...
    B2: ; Circle
        CJNE a, #0Eh, B3
        mov R0, #01h        ; DAC 1 Counter High Byte
        mov R1, #00h        ; DAC 1 Counter Low Byte
        mov R2, #01h        ; DAC 2 Counter High Byte
        mov R3, #00h        ; DAC 2 Counter Low Byte
        mov R4, #00h        ; DAC 1 Current Position High Byte
        mov R5, #00h        ; DAC 1 Current Position Low Byte
        mov R6, #40h        ; DAC 2 Current Position High Byte
        mov R7, #00h        ; DAC 2 Current Position Low Byte
        sjmp stoppoint
    B3: ; Special
        CJNE a, #0Dh, B4
        mov R0, #01h        ; DAC 1 Counter High Byte
        mov R1, #00h        ; DAC 1 Counter Low Byte
        mov R2, #04h        ; DAC 2 Counter High Byte
        mov R3, #00h        ; DAC 2 Counter Low Byte
        mov R4, #00h        ; DAC 1 Current Position High Byte
        mov R5, #00h        ; DAC 1 Current Position Low Byte
        mov R6, #00h        ; DAC 2 Current Position High Byte
        mov R7, #00h        ; DAC 2 Current Position Low Byte
        sjmp stoppoint
    B4: ; Rotating TIE
        CJNE a, #0Bh, stoppoint
        mov R0, #03h        ; DAC 1 Counter High Byte
        mov R1, #06h        ; DAC 1 Counter Low Byte
        mov R2, #01h        ; DAC 2 Counter High Byte
        mov R3, #00h        ; DAC 2 Counter Low Byte
        mov R4, #00h        ; DAC 1 Current Position High Byte
        mov R5, #00h        ; DAC 1 Current Position Low Byte
