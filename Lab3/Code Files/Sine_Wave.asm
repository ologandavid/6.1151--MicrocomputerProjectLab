...
main:
    mov R0, #0FFh           ; Initalize Counter for 256 bytes
    mov R1, #00h            ; Initialize Counter to Keep track of position in data table
    start:
        mov a, R1           ; Load Position in data table into acc
        mov dptr, #sine     ; Move address of sine table into data pointer
        movc A, @A+dptr     ; Get byte (sine value) at that location
        mov dptr, #0FE10h   ; Move DAC address into dptr
        movx @dptr, a       ; Move the sine value into the DAC
