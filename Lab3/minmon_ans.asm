...
; initialize hardware
   ...
   .db 0ah, 0dh, "Welcome to 6.115! :D", 0ah, 0dh,"MINMON> ", 0h
monloop:
...
jumtab:
   ...
   .dw read				; command 'r' 12 used
   .dw badcmd             ; command 's' 13
   .dw badcmd             ; command 't' 14
   .dw badcmd             ; command 'u' 15
   .dw vectortblwrite   ; command 'v' 16 used
   .dw write            ; command 'w' 17 used
...
;===============================================================
; command read  'r'
; this routine reads the contents of a given external memory location
;===============================================================
read:
	lcall getbyt			; get address high byte
	mov dph, a				; save in dph
	lcall prthex			; print the high byte to the PC
	lcall getbyt			; get address low byte
	mov dpl, a				; save in dpl
	lcall prthex			; print the low byte to the PC
	lcall crlf				; carriage return and line feed
	movx a, @dptr			; move the contents of the location
							; into A
	lcall prthex			; print the contents of A to the PC
	ljmp endloop			; return to monloop
	
	
;===============================================================
; command write  'w'
; this routine loads a hex byte into a specified external memory location
;===============================================================
write:
	lcall getbyt			; get address high byte
	mov dph,  a				; save in dph
	...
	mov dpl, a				; save in dpl
	lcall prthex			; print the low byte to the PC
	mov a, #3Dh				; move the ascii for '=' into A
	lcall sndchr			; print an '=' to the PC screen
	lcall getbyt			; get the data byte we would like to load in
	movx @dptr, a			; load that hex byte into the external mem loc
	lcall prthex			; print that data byte to the PC
	ljmp endloop			; return to monloop
	
	
;===============================================================
; command vectortblwrite 'v'
; this routine permits us to enter a table or vector of data bytes in external memory
;===============================================================
vectortblwrite:
	
	mov dpl, #00h			; store an initial low byte count in dpl
	lcall getbyt			; get address high byte
	lcall prthex
	mov dph, a				; save in dph
	vectorloop:
	mov r7, dpl				; store the low byte value in r7
	cjne r7, #0FFh, keepgoing	; if 256 values are not entered, we should read in the bytes
	done:
	ljmp endloop				; otherwise, we should stop
	keepgoing:
	lcall crlf				; issue carriage return and line feed
	mov a, dph				; store the high byte in A
	lcall prthex			; print the high byte to the PC again
	mov a, dpl				; store the low byte in A
	lcall prthex			; print the low byte to the PC
	
	mov a, #20h				; move the ascii for ' ' (space) into A
	lcall sndchr			; print a space to the screen
	lcall getbyt			; get the data byte we would like to load in
	; when we call getbyt, the msb is read in first, and is stored in B
	; we should check to see if that first byte entered is a return here
	mov r6, b				; store that byte in r6
	cjne r6, #0Dh, expandtable	; if b does not hold the return, let's read in the byte
	ljmp endloop			; otherwise we should stop	
	
	
	expandtable:
	movx @dptr, a			; load that hex byte into the external mem loc
	inc dptr				; increment the datapointer
	lcall prthex			; print that data byte to the PC
	sjmp vectorloop			; keep accepting vector table entries
